# Source: https://github.com/mjarosie/IdentityServerDockerHttpsDemo
...
$rootCN = "FlightLoggingSystemDevDockerRootCert"
...
if ($alreadyExistingCertsRoot.Count -eq 1) {
    ...
    $testRootCA = New-SelfSignedCertificate -Subject $rootCN -KeyExportPolicy Exportable -KeyUsageProperty Sign -KeyUsage CertSign -CertStoreLocation Cert:\LocalMachine\My
}
...
if ($alreadyExistingCertsIdentityServer.Count -eq 1) {
    ...
    $identityServerCert = New-SelfSignedCertificate -DnsName $identityServerCNs -Signer $testRootCA -CertStoreLocation Cert:\LocalMachine\My -KeyExportPolicy Exportable
}
...
if ($alreadyExistingCertsApi.Count -eq 1) {
    ...
    $webApiCert = New-SelfSignedCertificate -DnsName $webApiCNs -Signer $testRootCA -CertStoreLocation Cert:\LocalMachine\My -KeyExportPolicy Exportable
}
...
# $rootCertPathCer = "c:\temp\aspnetapp-root-cert.cer"
